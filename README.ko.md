...
<img src="assets/icon.png" width="128" alt="PikaTokenBar 아이콘">
...
2. 🐣 **부화.** [PokéAPI](https://pokeapi.co/)의 **1세대 모든 진화 계보(시작점 78종 — 나머지 73종은 진화로만 만날 수 있습니다)**에서 공식 capture rate 가중으로 태어납니다 — 흔한 포켓몬은 자주, 전설은 부화 195번에 1번. 부화한 포켓몬은 키우는 동안에도 **도감**에 바로 나타납니다. 부화마다 25종 성격 중 하나가 정해지고 — **아주 특별한 우연으론 ✨ 이로치가 태어납니다**.
...
<td width="55%" valign="middle">
<h3>설정에서 취향대로</h3>
메뉴바 표시 항목, 새로고침 간격(1–15분/수동), 로그인 시 자동 시작, 경고/임박 임계값 한도 알림, companion 이벤트 알림. <b>고급</b> 에서 claude.ai <code>sessionKey</code> 를 붙여넣으면 공식 한도를 <b>Keychain 프롬프트 없이</b> 조회합니다 — 자동 폴링까지 포함이라 stale 로 굳지 않습니다(한도 섹션만 숨기는 Keychain 끄기도 그대로 있습니다). <b>한국어/영어/일본어/스페인어/프랑스어/포르투갈어/독일어</b> UI·포켓몬 이름 완비.
</td>
</tr>
<tr>
<td width="55%" valign="middle">
...
<tr>
<td width="45%" align="center"><img src="assets/screenshot-pace-marker-ko.png" width="300" alt="총량이 아니라 페이스를 보기"></td>
<td width="55%" valign="middle">
<h3>총량이 아니라 페이스를 보기</h3>
각 한도 막대에는 고르게 썼다면 지금쯤 도달했을 지점에 얇은 눈금이 있습니다. 채움이 눈금을 넘으면 리셋 전에 한도가 바닥난다는 뜻입니다 — 4시간 남은 62%와 20분 남은 62%의 차이입니다. 창 길이를 알 수 없는 행에는 눈금이 없습니다. 막대와 퍼센트는 페이스에 따라 6단계로 색이 바뀝니다 — 한참 여유로우면 파랑, 페이스대로면 초록, 노랑·주황을 거쳐 한참 빠르면 빨강 — 어느 창을 아껴 써야 하는지 한눈에 보입니다.
</td>
</tr>
<tr>
<td width="45%" align="center"><img src="assets/screenshot-usage-recap-ko.png" width="300" alt="한 주의 사용량 돌아보기"></td>
<td width="55%" valign="middle">
<h3>지난 한 주·한 달·한 해 돌아보기</h3>
팝오버의 돌아보기 버튼을 누르면 이번 주·달·해를 포켓몬 도감 스타일 카드로 보여 줍니다 — 화살표로 이전 기간으로 넘길 수 있습니다. 합계, 날(또는 달)마다의 게이지, 이전 기간 대비 변화(진행 중이면 같은 일수끼리), 최고의 날, 최장 연속, 그 기간에 졸업한 포켓몬이 나옵니다. 기록은 올해와 작년분을 로컬에 보관합니다.
</td>
</tr>
</table>
...
- **이번 달 일별 추이** — 주간·월간 합계 아래에 이번 달 하루당 막대 하나. 날짜 축·주말 표시가 함께 있고, 가리킨 날(기본은 오늘)의 날짜·요일·사용량이 캡션에 나옵니다. 디스크에 이미 있는 로그에서 파생되므로 업데이트 후 첫 갱신에 곧바로 채워집니다.
...
- **Cursor 포함 사용량** — Cursor 에 로그인돼 있으면 공식 한도에 이번 달 포함(included) 사용량을 보여 줍니다: 사용률, 남은 금액, auto/API 구분.
- **이상한 사탕 한꺼번에 쓰기** — 여러 개를 한 번에 쓰고, 확정 전에 컴패니언이 얼마나 자랄지 미리 봅니다.
...
- **포켓몬 데이터와 에셋**은 런타임에 PokéAPI에서 받아오며 `~/Library/Application Support/PikaTokenBar/`에만 캐시됩니다. 생성된 개체값(IV·성별·특성·배운 기술 등)은 값이 변하지 않도록 로컬 파트너 세이브에 저장됩니다. 앱 바이너리와 릴리스 아티팩트에는 포켓몬 에셋이 포함되지 않습니다.
...
PikaTokenBar는 **비공식·비상업 팬 프로젝트**입니다. **Nintendo, Game Freak, Creatures Inc., The Pokémon Company와 제휴·보증·후원·승인 관계가 없습니다.** "포켓몬(Pokémon)"과 관련 명칭·캐릭터·이미지는 각 권리자의 상표 및 저작물이며, 본 프로젝트는 어떤 포켓몬 지식재산에 대해서도 소유권이나 권리를 주장하지 않습니다.
