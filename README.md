...
<img src="assets/icon.png" width="128" alt="PikaTokenBar icon">
...
| `session-key.json` → `claude.ai/api` | Claude official 5h/weekly % | optional: paste your claude.ai `sessionKey` cookie in Settings and limits refresh with **no Keychain prompt at all**, including auto-polling |
...
- **Pokémon data and assets** are fetched at runtime from PokéAPI and cached only under `~/Library/Application Support/PikaTokenBar/`. Generated individual values (such as IVs, gender, ability and known moves) are stored in the local companion save so they remain stable. The app binary and its release artifacts contain no Pokémon assets.
