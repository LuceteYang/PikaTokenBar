...
- **`set -o pipefail` 파이프에서, 뒤에서 일찍 끊는 리더(`grep -q`·`head`·`grep -m1`)는 리더 자신이 매치에
  ...
  *블로킹* 만 좌우하고, SIGPIPE 는 오직 리더가 닫혔는지에만 좌우된다(리더가 매치 직후 일찍 닫으면, writer 가
  ...
  확인은 검증이 아니다.
- **실패한 단계가 마지막 요약에서 성공으로 읽히면, 그 단계는 사실상 없는 것이다.** `release-fork.sh`
  8/8(cask 갱신)이 모든 실패 분기에서 `⚠` 한 줄만 찍고 흘러 내려가, 곧바로 `✓ vX 배포 완료` + brew
  설치 명령을 출력했다 — v1.1.0 실측: 방금 올린 자산을 다시 받다가 LibreSSL `bad decrypt`(errno 0)로
  단발 `curl` 이 죽어 cask 만 이전 버전에 남았는데, 요약은 완전 성공으로 보였고 **안내된 brew 명령은
  옛 버전을 설치하는 명령**이었다. 사람이 읽는 결과는 마지막 몇 줄뿐이라 그 위의 경고는 묻힌다.
  **왜 리뷰가 못 걸렀나**: 이 분기는 네트워크가 실패할 때만 밟혀 단위 테스트로 도달할 수 없고,
  정상 실행에서는 존재조차 보이지 않는다 — "실패 경로가 있다"와 "실패가 사람에게 도달한다"는 다른 질문이다.
  고치는 형태 둘: (1) 일시적 네트워크·TLS 오류는 **재시도로 흡수**한다(단발 `curl` 금지), (2) 단계
  성공 여부를 플래그로 끝까지 들고 가 **요약 자체를 분기**하고, 실패했으면 그 경로의 안내(brew 명령)를
  **출력하지 않는다** — 옛 버전을 깔라고 시키는 셈이기 때문이다. `exit 1` 로 죽이는 건 오답이다:
  릴리스는 이미 나갔는데 재실행으로 읽혀 universal 재빌드까지 되돌아간다. 회귀 가드: 셸 단계라
  단위 테스트가 없으므로 **결함 주입으로 확인한다** — 8/8 블록을 하네스로 떼어내 항상 실패하는
  `curl` 스텁을 PATH 앞에 두면 `✓ 배포 완료`가 사라지고 brew 줄이 안 나오는지, 로컬 bare tap 을
  `TAP_REPO` 로 주면 정상 경로가 여전히 `✓` 를 찍는지 양쪽을 본다.
...
- **메뉴바 상태아이템 = idle CPU 저격수 (두 규칙 필수).** 실측: 라이브 앱 idle ~14% CPU → 수정 후 ~2%.
  ...
  상주였고 fps 캡의 몫은 wakeup 을 프레임 수에 비례해 줄이는 것뿐이다(2026-08-20 에 22px 에서도 2.5fps 가
  느리다는 지적에서 출발해 0.4s→0.2s→**0.1s** 로 조정 — CA 전환이 제거된 뒤라 14% 당시와 같은 조건이
  아니다. 단 **fps 를 올리면 CPU 는 실제로 오른다**: 실측 0.2s/tol 0.5 = idle 1.8% → 0.1s/tol 0.1 = idle
  5.1%(2.8배). "CA 전환이 주범이었으니 fps 는 공짜"가 아니라, 주범이 사라진 만큼의 여유가 생긴 것뿐이다).
...
- **항상 뜬 애니메이션 표면은 메뉴바와 같은 idle 규율을 공유한다.** 플로팅 펫(`FloatingPetPanel`)처럼 상시
  ...
  **가드 비대칭 주의(2026-08-20 발견):** 펫 캡만 상수로 잠겨 있었고 메뉴바 캡은 인라인 리터럴
  `max(0.4, delay)` 여서 *캡이 통째로 사라져도 테스트가 못 잡았다*. 상시 표시 표면을 더할 땐 캡을 반드시
  **이름 있는 상수**로 두고 `>0` 가드를 붙인다(`testMenuFrameFloorIsCapped`,
  `testAlwaysVisibleSurfacesShareTheCapDiscipline` — 캡=0 주입으로 실패 확인). occlusion 게이팅은
