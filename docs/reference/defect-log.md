...
- **외부 데이터의 하드코딩 스냅샷에 단언하는 테스트는 그 데이터가 바뀌어도 계속 통과한다.** 트리거:
  1세대 전용 포크로 범위를 649→151 로 좁혔는데(커밋 `6e5d6ab`), `PremiumEggTests.testRareEggIsNotDominatedAtMeasuredPoolComposition`
  은 328종 시절 측정치를 하드코딩한 가중치 표에 대해서만 단언하고 있었다 — 풀이 78종으로 좁혀진 뒤에도
  그 옛 표를 계속 검증하며 통과해, 실제로는
  희귀 알이 고급 알 반복 구매보다 비싼 열등재가 된 걸 못 잡았다(Task 12). "테스트가 있는데 왜 못
  걸렀나"의 답: 트리거 조건(풀 범위 변경)과 테스트가 실제로 검증하는 대상(하드코딩된 숫자) 사이에
  연결이 없어서다 — 코드는 실행 중인 인덱스를 읽지만 테스트는 그 인덱스를 절대 보지 않는다.
  **부류 스윕 결과**: `Tests/` 전체에서 `capture_rate` 합·측정 풀 가중치를 테이블로 박아 둔 곳은 이
  테스트 하나뿐(`Gen1RangeTests.swift` 등 다른 곳은 종수만 근사치로 주석에 적을 뿐 가중치 표는 없음)
  — 같은 부류의 다른 인스턴스는 없다. **영구 캡처**: 이 테스트 안에 범위 자체를 고정하는 가드를
  추가했다 — `XCTAssertEqual(PokemonAssets.animatedSpeciesIDs, 1...151, ...)`. 범위가 다시 바뀌면(예:
  2세대 확장) 가중치 표를 보기도 전에 이 단언이 먼저 깨져, "옛 표로 계속 통과"를 원천적으로 막는다.
  같은 부류를 새로 만들 때는(측정 데이터를 테이블로 박아 테스트에 넣는 경우) 그 측정이 유효한
  **범위/스코프를 정하는 상수**를 찾아 같은 방식으로 핀을 박아라 — 기억(주석)이 아니라 코드로 막는다.
