...
APP="/Applications/PikaTokenBar.app"
LOG="$HOME/Library/Logs/PikaTokenBar.log"
SNAP="$HOME/Library/Application Support/PikaTokenBar/last-snapshot.json"
...
sleep 2
INSTANCES=$(pgrep -x PikaTokenBar | wc -l | tr -d ' ')
case "$INSTANCES" in
  0) bad "프로세스 없음"; exit 1 ;;
  1) ok "프로세스 생존, 단일 인스턴스 (pid $(pgrep -x PikaTokenBar))" ;;
  ...
  *) bad "인스턴스 $INSTANCES 개 — 중복 실행 (pid $(pgrep -x PikaTokenBar | tr '\n' ' '))" ;;
esac
...
var status = 0, tall = 0
for w in list where (w[kCGWindowOwnerName as String] as? String) == "PikaTokenBar" {
...
AXCNT=$(osascript -e 'tell application "System Events" to tell process "PikaTokenBar" to get count of menu bar items of menu bar 2' 2>/dev/null)
if [[ "${AXCNT:-}" =~ ^[0-9]+$ ]]; then
...
press() { osascript -e 'tell application "System Events" to tell process "PikaTokenBar" to perform action "AXPress" of menu bar item 1 of menu bar 2' >/dev/null 2>&1; }
