...
final class CompanionStore {
    ...
    static func defaultURL() -> URL {
        // 상태 파일 위치. `AppStatePaths` 는 포크에서 `AppIdentity.stateDirectory` 로 위임하므로
        // 기본은 Application Support/<AppIdentity.supportDirectoryName>, `PTB_STATE_DIR` 이 있으면
        // 그 디렉토리 — 환경변수 해석은 AppIdentity 가 전담한다(직독 금지).
