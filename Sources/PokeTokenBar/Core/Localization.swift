...
struct L {
    ...
    // MARK: claude.ai 세션 키 (Keychain 프롬프트 없는 한도 경로) — 포크 전용
    var sessionKeyLabel: String { t("claude.ai 세션 키", "claude.ai session key", "claude.ai セッションキー", "Clave de sesión de claude.ai", "Clé de session claude.ai", "Chave de sessão do claude.ai") }
    ...
    var sessionKeySaved: String { t("설정됨", "Saved", "設定済み", "Guardada", "Enregistrée", "Salva") }
    var save: String { t("저장", "Save", "保存", "Guardar", "Enregistrer", "Salvar") }
    var delete: String { t("삭제", "Delete", "削除", "Eliminar", "Supprimer", "Excluir") }
    var sessionKeyOrganizationLabel: String { t("조직", "Organization", "組織", "Organización", "Organisation", "Organização") }
    ...
    var reportAttachHint: String {
        t("이슈에 로그 파일을 첨부해 주시면 원인 파악에 큰 도움이 돼요.",
          ...
          "Adjuntar el archivo de registro a la incidencia ayuda mucho a diagnosticar el problema.",
          "Joindre le fichier journal à l'issue aide beaucoup au diagnostic.",
          "Anexar o arquivo de log à issue ajuda muito no diagnóstico.")
    }
    ...
    func reportIssueFallback(_ urlString: String) -> String {
        t("브라우저를 열 수 없어요. \(urlString) 로 직접 접속해 주세요.",
          ...
          "No se pudo abrir un navegador. Visita \(urlString) directamente.",
          "Impossible d'ouvrir un navigateur. Va directement sur \(urlString).",
          "Não foi possível abrir um navegador. Acesse \(urlString) diretamente.")
    }
    func reportIssueTitle(_ version: String) -> String {
        t("[\(AppIdentity.executableName)] 문제 리포트 (v\(version))",
          ...
          "[\(AppIdentity.executableName)] Reporte de problema (v\(version))",
          "[\(AppIdentity.executableName)] Rapport de problème (v\(version))",
          "[\(AppIdentity.executableName)] Relato de problema (v\(version))")
    }
    func reportIssueBody(version: String, os: String) -> String {
        ...
        macOS: \(os)
        Archivo de registro (se recomienda adjuntar): ~/Library/Logs/\(AppIdentity.logFileName)
        """,
        """
        Ce qui s'est passé :
        ...
        macOS: \(os)
        Fichier journal (à joindre de préférence) : ~/Library/Logs/\(AppIdentity.logFileName)
        """,
        """
        ...
        macOS: \(os)
        Arquivo de log (anexe, por favor): ~/Library/Logs/\(AppIdentity.logFileName)
        """)
    }
