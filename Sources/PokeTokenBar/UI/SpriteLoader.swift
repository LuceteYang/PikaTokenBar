...
actor SpriteStore {
    ...
    /// private 아님(테스트가 `SpriteLoader.cacheDir` 와 같은 폴더인지 검증) — 그 외엔 주입 전용.
    nonisolated let directory: URL

    /// 기본 경로는 반드시 `spriteCacheDirectory()` 다. upstream 은 여기서
    /// `PokeTokenBar/sprites` 를 하드코딩하는데, 그대로 받으면 원본 앱과 같은 캐시 폴더를
    /// 공유하게 된다. 인자는 테스트 주입용.
    init(directory: URL? = nil) {
        let d = directory ?? spriteCacheDirectory()
        self.directory = d
